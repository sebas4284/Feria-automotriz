    ...
    <div class="flex items-start justify-between">
        ...
        <div class="flex items-center gap-2 shrink-0">
            @if(auth()->user()->isAdmin())
                <a href="{{ route('ventas.analisis') }}" title="Análisis de ventas"
            ...
            @if(auth()->user()->isConcesionario())
                <a href="{{ route('ventas.mi-analisis') }}" title="Mi análisis"
                    class="w-11 h-11 bg-gray-800 hover:bg-gray-700 rounded-2xl flex items-center justify-center transition">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M3 13.125C3 12.504 3.504 12 4.125 12h2.25c.621 0 1.125.504 1.125 1.125v6.75C7.5 20.496 6.996 21 6.375 21h-2.25A1.125 1.125 0 0 1 3 19.875v-6.75ZM9.75 8.625c0-.621.504-1.125 1.125-1.125h2.25c.621 0 1.125.504 1.125 1.125v11.25c0 .621-.504 1.125-1.125 1.125h-2.25a1.125 1.125 0 0 1-1.125-1.125V8.625ZM16.5 4.125c0-.621.504-1.125 1.125-1.125h2.25C20.496 3 21 3.504 21 4.125v15.75c0 .621-.504 1.125-1.125 1.125h-2.25a1.125 1.125 0 0 1-1.125-1.125V4.125Z" />
                    </svg>
    ...
    <div>
        ...
        <div class="space-y-2">
            @forelse($ventas as $venta)
                <div x-show="matches($el)" data-search="{{ mb_strtolower(($venta->comprador->nombre ?? '').' '.$venta->vehiculo->marca.' '.$venta->vehiculo->modelo.' '.$venta->vehiculo->placa.' '.($venta->vehiculo->concesionario->nombre ?? '').' '.($venta->concesionarioVende->nombre ?? '').' '.($venta->asesorComercial->nombre ?? '')) }}"
        ...
        <table class="w-full">
            <thead class="bg-gray-800">
                <tr>
                    ...
                    <th class="p-4 text-left hidden lg:table-cell">Dueño</th>
                    <th class="p-4 text-left hidden lg:table-cell">Vendedor</th>
            ...
            <tbody>
                @forelse($ventas as $venta)
                    <tr x-show="matches($el)" data-search="{{ mb_strtolower(($venta->comprador->nombre ?? '').' '.$venta->vehiculo->marca.' '.$venta->vehiculo->modelo.' '.$venta->vehiculo->placa.' '.($venta->vehiculo->concesionario->nombre ?? '').' '.($venta->concesionarioVende->nombre ?? '').' '.($venta->asesorComercial->nombre ?? '')) }}"
                        ...
                        <td class="p-4 hidden lg:table-cell">{{ $venta->vehiculo->concesionario->nombre ?? '—' }}</td>
                ...
                @empty
                    <tr>
                        <td colspan="9" class="text-center p-8 text-gray-400">No hay ventas registradas</td>
                    </tr>
                @endforelse
                <tr x-show="q.trim() !== '' && visibleCount === 0">
                    <td colspan="9" class="text-center p-8 text-gray-400">Sin resultados para tu búsqueda</td>
                </tr>
            </tbody>
        </table>
    </div>
