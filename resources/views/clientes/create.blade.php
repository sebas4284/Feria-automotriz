        ...
        <form action="{{ route('clientes.store') }}" method="POST"
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" name="vehiculo_interes" value="{{ old('vehiculo_interes') }}"
                        class="w-full bg-gray-800 border border-gray-700 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-blue-500">
            ...
            <div class="mt-6">
                ...
                <select name="estado" class="w-full bg-gray-800 border border-gray-700 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-blue-500">
                    <option value="Nuevo" @selected(old('estado') === 'Nuevo')>Nuevo</option>
                    <option value="Contactado" @selected(old('estado') === 'Contactado')>Contactado</option>
                    <option value="Negociacion" @selected(old('estado') === 'Negociacion')>Negociación</option>
                    <option value="Vendido" @selected(old('estado') === 'Vendido')>Vendido</option>
                    <option value="Perdido" @selected(old('estado') === 'Perdido')>Perdido</option>
                </select>
                @error('estado')
                    <p class="text-red-400 text-xs mt-1">{{ $message }}</p>
                @enderror
